...
const app = express()
const port = 2132;
const API_KEY = 'adxcv-zzadq-polkjsad-opp13opoz-1sdf455aadzmck1244567'
...
app.get('/pay', (req, res) => {
    ...
    form.append("api", API_KEY) //your_api -> required
    form.append("amount", 10000) //your_amount -> required
    form.append("redirect", `http://${IP}:${port}/pay/get_redirect`) //your_redirect -> required
    ...
    axios({
        ...
        .then(function (response) {
            ...
            if (response.data === -1)
                ...
                res.redirect(`https://bitpay.ir/payment-test/gateway-${response.data}-get`)
            }
        })
        .catch(err => {
            console.log(err)
...
app.post('/pay/get_redirect', (req, res) => {
